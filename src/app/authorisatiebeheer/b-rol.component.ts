...
addCursus() {
  ...
  this.authService.maakTokenHeadervoorCurcon().then( token => {
      this.rolService.addRole(this.userForm.email,token).subscribe(user => {
        this.mode = 'view';
        this.rolService.getRoles(token).subscribe(cursussen => {
...
saveCursus(form: any) {
  ...
  this.authService.maakTokenHeadervoorCurcon().then( token => {
    ...
    this.rolService.updateRole(this.selectedCursus.id, form.value, token).subscribe(data => {
      this.mode = 'view';
      this.loadcursussen();
      this.rolService.getRolesByObject(this.selectedCursus,token).subscribe(cursus => {
        console.log(cursus);
...
addModuleToRol(form: any) {
    this.loading = true;
this.authService.maakTokenHeadervoorCurcon().then( token => {
      this.rolService.addModuleToRole(this.selectedCursus.id, form, token).subscribe(data => {
...
deleteBeroepstaak(md: Object) {
  this.authService.maakTokenHeadervoorCurcon().then( token => {
    this.rolService.deleteModuleFromRole(this.selectedCursus.id, md['id'], token).subscribe(
