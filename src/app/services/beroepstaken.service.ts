...
export class BeroepstakenService {
   ...
   getBeroepstaken() {
      return this.http.get(myGlobals.baseUrl+'beroepstaken/').map(res => res.json());
   }

   getBeroepstaakTypes() {
      return this.http.get(myGlobals.baseUrl + 'beroepstaken/types').map(res => res.json());
   }

   getBeroepstakenByObject(obj) {
      return this.http.get(obj.href).map(res => res.json());
   }
