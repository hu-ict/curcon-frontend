...
export class ToetsenService {
  ...
  getDataByHref(href) {
	...
	saveBeoordelingsElement(toetsId, element) {
		if (element.id == null) {
			return this.http.post<curconnamespace.CurconNameSpace.>(myGlobals.baseUrl+'toetsen/' + toetsId + '/beoordelingselementen', element)
			.pipe(catchError(this.handleError));
		} else {
			var elementId = element.id;
			delete element.id;
			return this.http.put<curconnamespace.CurconNameSpace.>(myGlobals.baseUrl+'beoordelingselementen/' + elementId, element)
			.pipe(catchError(this.handleError));
		}
	}
